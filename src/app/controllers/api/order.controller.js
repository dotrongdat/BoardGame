...
import {
	...
	orderStatus,
	paymentMethodType,
	vnp_ResponseCode as vnp_ResponseCodeConstant,
...
const checkConfirmCart = async (confirmCart) => {
	const products = await Product.find({ _id: Object.keys(confirmCart) })
	...
	try {
		...
		switch (order.statuses[order.statuses.length - 1]) {
			...
			case orderStatus.CANCEL_BY_STORE:
				...
				notificationData.content =
					'Đơn hàng đã bị hủy bởi cửa hàng vì một số lí do'
				break
	...
	try {
		...
		promiseArray.push(
			pushOrderNotification(
				order,
				userId,
				order.user ? order.user._id : undefined
			)
		)
...
const createOrder = async (
	{ code, products, info, message, total },
	userId = undefined
) => {
	let orderData = {
		code,
	...
	sendMail({
		to: order.info.email,
		subject: `Hunter đã nhận đơn hàng #${order.code}`,
		html: mailHtml,
	})
	return order
}
...
const checkout = async (req, res) => {
	try {
		...
		let productsInStore = await Product.find({ _id: Object.keys(confirmCart) })
			.select('_id quantity price status')
			.exec()
		let isValid = true
		...
		if (isValid) {
			...
			switch (info.paymentMethod.type) {
				case paymentMethodType.VNPAY:
					...
					global.nodeCache.set(
						`vnpay${code}`,
						{ ...newOrder, userId },
						VNPAY_CODE_EXPIRE_IN
					)
					return res.status(statusCode.OK).json({
...
const get = async (req, res) => {
	try {
		...
		if (_id) {
			...
			order = await Order.find({ _id: user.order }).lean()
		}
		payload.order = order
	...
	try {
		...
		const dbQueries = status.map((s) => {
			...
			if (date) {
				...
				condition.logDate[s] = {
					$and: [{ $gte: beginOfDate }, { $lte: endOfDate }],
				}
			}
			return Order.find(condition).populate('user', '_id name image').lean()
			// })
		})
...
const cancelOrder = async (req, res) => {
	try {
		...
		const adminUserId = await User.findOne({ order: order._id })
			.select('_id')
			.lean()
...
const updateStatus = async (req, res) => {
	try {
		...
		// }
		if (order.user)
			promiseArray.push(pushOrderNotification(order, user._id, order.user))
		promiseArray.push(
...
export { matchOrderToSpecificAdminUser }
export default {
	checkout,
	validateConfirmCart,
	get,
